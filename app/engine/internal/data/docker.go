...
func (r *dockerRepo) CreateContainer(ctx context.Context, c *biz.Container) (id string, warnings []string, err error) {
	...
	/* TODO: 容器信息入库管理 */
	/* 获取容器信息 */
	containerInfo, err := r.InspectContainer(ctx, id)
	if err != nil {
		return "", nil, err
	}
	/* 容器记录入库 */
	...
	if _, err := r.data.entDB.Container.Create().
...
func (r *dockerRepo) OssGetFile(url string) ([]byte, error) {
	return nil, nil
}

// ParseComposeFile parses a compose file and returns []*biz.ContainerConfig
func (r *dockerRepo) ParseComposeFile(ctx context.Context, fp []byte) ([]*biz.ContainerConfig, error) {
